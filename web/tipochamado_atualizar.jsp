...
<html>
    ...
    <body>
        ...
        <div class="container ">
            <%
               TipoChamado t = (TipoChamado) request.getAttribute("tipos");
                String mensagem = (String) request.getAttribute("mensagem");
            %>

            <form role="form" class="form-horizontal" action="AlterarTipoChamado" method="post">
                <div class="row">
                    <div class="form-group">
                        </br></br></br>
                        <h3>Dados do Tipo de Chamado </h3>
                    </div>
                    <div class="col-md-6 mb-3" >
                        <label class="control-label">Nome: </label>
                        <input type="hidden" name="id" value="<%=t.getId()%>">
                        <input type="text" class="form-control" name="tch_nome" placeholder="Nome do tipo do Chamado" value = "<% if (t != null)%> <%= t.getTch_nome()%>"  />
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class=" control-label">Status:</label>
                        <select class="form-control" name="tch_status">
                            <option value="">Selecione</option>
                            <option value="Ativo" <% if (t.getTch_status().equals("Ativo")) {%> selected="" <% } %>>Ativo</option>
                            <option value="Inativo" <% if (t.getTch_status().equals("Inativo")) {%> selected="" <% } %>>Inativo</option>
                        </select>
                    </div>
                    <div class="col-md-12 mb-3">
                        <label class=" control-label">Descricao: </label>
                        <input type="text" class="form-control" name="tch_descricao" placeholder="Descrição do tipo do Chamado" value = "<% if (t != null)%> <%= t.getTch_descricao()%>"   />
                        </div>
                    </div>
                    </br>
                    <div class="row">
                    <div class="form-group">
                        <h3>Dados da SLA por Severidade do Tipo de Chamado (em horas) </h3>
                    </div> 
                    <div class="col-md-4 mb-3">
                        <label class=" control-label">Normal: </label>
                        <input type="text" class="form-control" name="tch_normal" placeholder="valor em horas serveridade normal" value = "<% if (t != null)%> <%= t.getTch_normal()%>"  />
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class=" control-label">Urgente: </label>
                        <input type="text" class="form-control" name="tch_urgente" placeholder="valor em horas serveridade urgente" value = "<% if (t != null)%> <%= t.getTch_urgente()%>"  />
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class=" control-label">Critico: </label>
                        <input type="text" class="form-control" name="tch_critico" placeholder="valor em horas serveridade critico" value = "<% if (t != null)%> <%= t.getTch_critico()%>"   />
                    </div>
                   </div>
                    <div class="row">
